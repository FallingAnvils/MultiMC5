...
{
    ui->setupUi(this);
    auto channel = MSMC->settings()->get("UpdateChannel").toString();
    if(hasUpdate)
    {
    ...
    loadChangelog();
    restoreGeometry(QByteArray::fromBase64(MSMC->settings()->get("UpdateDialogGeometry").toByteArray()));
}
...
void UpdateDialog::loadChangelog()
{
    ...
    if(channel == "stable")
    {
        url = QString("https://raw.githubusercontent.com/MultiServerMC/MultiServerMC5/%1/changelog.md").arg(channel);
        m_changelogType = CHANGELOG_MARKDOWN;
    }
    else
    {
        url = QString("https://api.github.com/repos/MultiServerMC/MultiServerMC5/compare/%1...%2").arg(BuildConfig.GIT_COMMIT, channel);
...
QString reprocessMarkdown(QByteArray markdown)
{
    ...
    output.replace(QRegExp("GH-([0-9]+)"), "<a href=\"https://github.com/MultiServerMC/MultiServerMC5/issues/\\1\">GH-\\1</a>");
    qDebug() << output;
    return output;
}
    ...
    try
    {
        ...
        auto print_commits = [&]()
        {
            ...
            for(int i = commitarray.size() - 1; i >= 0; i--)
            {
                ...
                if(issuenr.length())
                {
                    result += QString("<a href=\"https://github.com/MultiServerMC/MultiServerMC5/issues/%1\">GH-%2</a>").arg(issuenr, issuenr);
                }
...
{
    MSMC->settings()->set("UpdateDialogGeometry", saveGeometry().toBase64());

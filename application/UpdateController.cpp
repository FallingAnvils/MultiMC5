...
void UpdateController::installUpdates()
{
    ...
    QString finishCmd = FS::PathCombine(m_root, "MultiServerMC");
    ...
    for(auto op: m_operations)
    {
        switch(op.type)
        {
            ...
            case GoUpdate::Operation::OP_REPLACE:
            {
                ...
                if(op.destination == "MultiServerMC.exe")
                {
                ...
                {
                    if(destination.fileName() == "MultiServerMC.exe")
                    {
                        QDir rootDir(m_root);
    ...
    switch (m_failedOperationType)
    {
        case Replace:
        {
            msg = QObject::tr("Couldn't replace file %1. Changes will be reverted.\n"
                "See the MultiServerMC log file for details.").arg(m_failedFile);
            doRollback = true;
    ...
    if(doRollback)
    {
        ...
        if(!rollbackOK)
        {
            msg = QObject::tr("The rollback failed too.\n"
                "You will have to repair MultiServerMC manually.\n"

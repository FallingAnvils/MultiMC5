...
void LoggedProcess::on_exit(int exit_code, QProcess::ExitStatus status)
{
    ...
    if (!m_is_aborting)
    {
        if (status == QProcess::NormalExit)
        {
            ...
            emit log({tr("Process exited with code %1.").arg(exit_code)}, MessageLevel::MultiServerMC);
        ...
        else
        {
            ...
            if(exit_code == -1)
                emit log({tr("Process crashed.")}, MessageLevel::MultiServerMC);
            else
                emit log({tr("Process crashed with exitcode %1.").arg(exit_code)}, MessageLevel::MultiServerMC);
